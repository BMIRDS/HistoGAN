...
def rename(shuffled_keys):
    ...
    # key is real image name (e.g. "real/no_alk_123.jpg")
    # the value is the new name to use (e.g. aa.png)
    result = {key: str(i) for i, key in enumerate(shuffled_keys, 1)}
    return result
...
def generate_turing_test(real_image_folder, synthetic_images_folder, output_folder,
                         control_image_folder, baseline_1, baseline_2):
    # Dictionaries to record data

    # the key is real image name (e.g. "real/no_alk_123.jpg")
    # the value is "R" or "F" or "C"
    real_or_fake_dictionary = {}
    # the key is real image name (e.g. "real/no_alk_123.jpg")
    # the value is the new name to use (e.g. aa.png)
    original_and_renamed_dictionary = {}

    # Loop through real images and register them as real
    for image in os.listdir(real_image_folder):
        image_path = join(real_image_folder, image)
    ...
    for image in os.listdir(synthetic_images_folder):
        image_path = join(synthetic_images_folder, image)
    ...
    for image in os.listdir(control_image_folder):
        image_path = join(control_image_folder, image)
    ...
    with open('pathologist_file.csv', mode='w') as file:
        ...
        for each in shuffled_keys:
            ...
            if '.DS_Store' in each:
                continue
            os.system("cp -r {} {}.png".format(
                each, join(output_folder, original_and_renamed_dictionary.get(each))))


# Whether an image is mostly whitespace, takes in np array
def is_whitespace(crop):
    pooled = skimage.measure.block_reduce(
        crop, (int(crop.shape[0]/10), int(crop.shape[1]/10), 3), np.average)
...
# Gets N images from the folder (synthetic images should have the same name as
# real images they were generated from but be in different folder)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--real_image_folder", type=str,
                        help="path to the folder with real images")
    parser.add_argument("--fake_image_folder", type=str,
                        help="path to the folder with fake images")
    parser.add_argument("--output_folder", type=str,
                        help="where to output the renamed images")
    parser.add_argument("--num_images", type=int,
                        help="N number of images to use per class (total of 2N images)")
    parser.add_argument("--operation_num", type=int,
                        help="0 to generate Turing Test, 1 to get N images from folder")
    parser.add_argument("--control_image_folder", type=str,
                        help="path to the folder with control images")
    parser.add_argument("--baseline_1_folder", type=str,
                        help="path to the folder with first baseline images")
    parser.add_argument("--baseline_2_folder", type=str,
                        help="path to the folder with second baseline images")
    args = parser.parse_args()

    if args.operation_num is 1:
        get_best_images(
            args.real_image_folder,
            args.fake_image_folder,
            args.num_images,
            args.output_folder)
    elif args.operation_num is 0:
        generate_turing_test(
            args.real_image_folder,
            args.fake_image_folder,
            args.output_folder,
            args.control_image_folder,
            args.baseline_1_folder,
            args.baseline_2_folder)
    else:
