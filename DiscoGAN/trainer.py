    ...
    def build_model(self):
        if self.dataset == 'toy':
            ...
            if self.cnn_type == 0:
                ...
                conv_dims, deconv_dims = [64, 128, 256, 512, 1024, 2048], [1024, 512, 256, 128, 64]
        ...
        for step in trange(self.start_step, self.max_step):
            ...
            if step % self.log_step == 0:
                print("[{}/{}] Loss_D: {:.4f} Loss_G: {:.4f}". \
                      format(step, self.max_step, l_d.data, l_g.data))

                print("[{}/{}] l_d_A_real: {:.4f} l_d_A_fake: {:.4f}, l_d_B_real: {:.4f}, l_d_B_fake: {:.4f}". \
                      format(step, self.max_step, l_d_A_real.data, l_d_A_fake.data,
                             l_d_B_real.data, l_d_B_fake.data))

                print("[{}/{}] l_const_A: {:.4f} l_const_B: {:.4f}, l_gan_A: {:.4f}, l_gan_B: {:.4f}". \
                      format(step, self.max_step, l_const_A.data, l_const_B.data,
                             l_gan_A.data, l_gan_B.data))
